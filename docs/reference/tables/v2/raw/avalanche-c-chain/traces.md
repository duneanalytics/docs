...
| refund\_address | string       | only contains data if `type` was `suicide`. Specifies where to send the outstanding avax balance.                                                                                                                                             |

### Gas used in traces

The `gas_used` column in the avax\_c.traces table is a bit hard to understand, so here is some pointers:
