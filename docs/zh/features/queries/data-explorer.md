...
数据浏览器使您能够搜索区块链和其他数据，以便在您的查询中使用（在[表格部分](../../tables/index.md)了解 Dune 提供的所有数据）。
...
这将带来一个包含区块链数据的[表格](../../tables/index.md)列表，您可以将其用于您的查询！🧙
...
=== "V2"
    ...
    2. [**已解析项目**](../../tables/decoded.md) —— Dune 团队和社区的项目的合约调用和事件已被整理成不错的人类可读的表格
